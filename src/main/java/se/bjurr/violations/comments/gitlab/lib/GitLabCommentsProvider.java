...
public class GitLabCommentsProvider implements CommentsProvider {
	...
	/**
	 * Set the {@link GitlabMergeRequest} as "Work in Progress" if configured
	 * to do so by the shouldSetWIP flag.
	 */
	...
	public void createDiffDiscussion(final ChangedFile file,
		...
		try {
			...
			LOGGER.error("Could not create diff discussion!" + lineSeparator +
					"ProjectID: " + projectId + lineSeparator +
					"SourceSha: " + sourceSha + lineSeparator +
	...
	public void removeComments(final List<Comment> comments) {
		for (final Comment comment : comments) {
			try {
				...
				LOGGER.error("Could not delete note " + comment);
			}
		}
	}
